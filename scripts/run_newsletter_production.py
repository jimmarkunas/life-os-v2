...
def _load_private_policy(path: str) -> tuple[LaneConfig, dict[str, int], FitProfile, str, str]:
    """Synthetic-test fixture loader; the live path never requires this file."""
    try:
    ...
    if not isinstance(raw, dict):
        raise ProductionConfigError("private policy fixture was not an object")
...
def _policy_file_url(page: dict) -> str:
    ...
    if len(files) != 1 or not isinstance(files[0], dict):
        raise ProductionConfigError("US Remote private policy file is missing or ambiguous")
    entry = files[0]
    payload = entry.get("file") if entry.get("type") == "file" else entry.get("external")
    url = payload.get("url") if isinstance(payload, dict) else None
    if not url:
        raise ProductionConfigError("US Remote private policy file has no readable URL")
    return str(url)


def _load_private_policy_from_notion(
    context: RunContext,
    http: HttpClient,
    notion: NotionTransport,
    *,
    notion_token: str,
) -> tuple[LaneConfig, dict[str, int], FitProfile, str, str]:
    search = http.request_json(
        context,
        "POST",
        _NOTION_SEARCH_URL,
        headers={
    ...
    if not isinstance(search, dict):
        raise ProductionConfigError("Notion configuration search returned an invalid response")
    matches = [
        item
        for item in (search.get("results") or [])
        if isinstance(item, dict)
        and _plain_title(item) == _POLICY_DATABASE_TITLE
        and item.get("id")
    ]
    if len(matches) != 1:
        raise ProductionConfigError("canonical Job Lane Configuration data source was not uniquely resolvable")
    rows = notion.query_data_source(
        str(matches[0]["id"]),
        NotionIdentityQuery(property_name="Lane", property_type="title", values=(_POLICY_LANE,)),
        page_size=10,
    )
    if len(rows) != 1:
        raise ProductionConfigError("canonical US Remote lane was not uniquely resolvable")
    raw = http.request_json(
        context,
        "GET",
        _policy_file_url(rows[0]),
        timeout_seconds=10.0,
        retry=RetryPolicy(max_attempts=2, backoff_seconds=0.1, max_backoff_seconds=1.0),
    )
    if not isinstance(raw, dict):
        raise ProductionConfigError("canonical private policy was not a JSON object")
    return _parse_private_policy(raw)
