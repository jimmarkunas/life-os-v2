...
REQUIRED_ENV = ("GMAIL_OAUTH_CLIENT_ID", "GMAIL_OAUTH_CLIENT_SECRET", "GMAIL_OAUTH_REFRESH_TOKEN", "NOTION_API_TOKEN", "NOTION_JOB_LEDGER_DATA_SOURCE_ID")

class ProductionConfigError(RuntimeError): pass

def _require_env() -> dict[str, str]:
    missing = [name for name in REQUIRED_ENV if not os.getenv(name)]
    if missing: raise ProductionConfigError("missing required runtime configuration: " + ", ".join(missing))
...
def _parse_private_policy(raw: dict) -> tuple[LaneConfig, dict[str, int], FitProfile, str, str]:
    try:
        ...
        lane_priority = {str(k): int(v) for k, v in raw["lane_priority"].items()}; fit_raw = raw["fit_profile"]
    ...
    except (KeyError, TypeError, ValueError) as exc: raise ProductionConfigError(f"private policy is invalid: {type(exc).__name__}") from exc
...
def _load_private_policy(path: str) -> tuple[LaneConfig, dict[str, int], FitProfile, str, str]:
    ...
    except (OSError, json.JSONDecodeError) as exc: raise ProductionConfigError(f"could not read private policy fixture: {type(exc).__name__}") from exc
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if fixture_path: lane, lane_priority, fit_profile, market, source_lane = _load_private_policy(fixture_path)
        else: lane, lane_priority, fit_profile, market, source_lane = _load_private_policy_from_notion(context, http, notion_transport, notion_token=env["NOTION_API_TOKEN"])
