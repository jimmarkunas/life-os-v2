...
def test_get_many_returns_only_requested_keys_present():
    ...
    opp = Job(stable_job_key="k1", job=make_job(), admission_status=AdmissionStatus.ADMITTED)
...
def test_no_full_scan_required_for_narrow_lookup():
    ...
    repo = InMemoryCareerRepository()
    for i in range(50):
        opp = Job(stable_job_key=f"k{i}", job=make_job(), admission_status=AdmissionStatus.ADMITTED)
        repo.upsert(new_record(opp, run_date=RUN_DATE))
