...
def _canonical_job(key="k1", admission=AdmissionStatus.ADMITTED):
    return Job(stable_job_key=key, job=make_job(), admission_status=admission)


def test_new_record_starts_new_and_live():
    record = new_record(_canonical_job(), run_date=RUN_DATE)
    assert record.status == LifecycleStatus.NEW
    assert record.live is True
    assert record.first_surfaced == RUN_DATE


def test_excluded_job_starts_historical_not_live():
    record = new_record(_canonical_job(admission=AdmissionStatus.EXCLUDED), run_date=RUN_DATE)
...
def test_apply_observation_preserves_first_surfaced():
    original = new_record(_canonical_job(), run_date=RUN_DATE)
    later = RUN_DATE + timedelta(days=5)
    updated = apply_observation(original, _canonical_job(), run_date=later)
...
def test_applied_flag_is_preserved_across_reobservation():
    ...
    reobserved = apply_observation(applied, _canonical_job(), run_date=RUN_DATE + timedelta(days=10))
...
def test_review_transition_after_ready_date():
    original = new_record(_canonical_job(), run_date=RUN_DATE)
    reviewed = apply_observation(original, _canonical_job(), run_date=original.review_ready_on)
...
def test_absence_is_never_a_close_reason():
    ...
    only calling apply_observation with a live Job keeps status current. This
    test documents the invariant by construction: there is no API path from
    "not observed this run" to closed."""
    original = new_record(_canonical_job(), run_date=RUN_DATE)
...
def test_apply_observation_rejects_mismatched_key():
    original = new_record(_canonical_job(key="k1"), run_date=RUN_DATE)
    with pytest.raises(ValueError):
        apply_observation(original, _canonical_job(key="k2"), run_date=RUN_DATE)
