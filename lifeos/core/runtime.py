...
# Execution-wide ceiling on concurrently active backend HTTP calls. Every
# HttpClient.request() under one RunContext -- regardless of which domain's
# thread pool issued it (Gmail, Outlook, Notion, Jobs terminal evidence) --
# competes for this same bounded budget. Deliberately generic: Core owns the
# ceiling, never domain-specific worker-count policy.
...
class RunContext:
    ...
    def start(
        ...
        *,
        ...
        return cls(
            ...
            _http_permits=threading.Semaphore(int(http_concurrency)),
        )
    ...
    def http_permit(self) -> Iterator[None]:
        """Bound execution-wide concurrently active HTTP calls. Blocks until a
        permit is free, but never past this RunContext's own deadline -- a
        thread that would otherwise wait past the deadline instead raises
        DeadlineExceeded, the same fail-closed signal as every other bounded
        wait in this module. Not a queue/scheduler: this is a single stdlib
        semaphore scoped to one execution."""
