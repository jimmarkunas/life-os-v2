...
class NotionCareerRepository:
    ...
    def upsert_many(self, records: list[JobLedgerRecord]) -> dict[str, JobLedgerRecord]:
        ...
        def _write_one(item: tuple[str, JobLedgerRecord]) -> None:
            key, record = item
            properties = _record_to_properties(record)
            with self._page_ids_lock:
            ...
            if page_id:
                try:
                    ...
                    if _canonical_view(observed) == _canonical_view(record): return
                    written = self._transport.update_page(page_id, properties)
                with self._page_ids_lock:
        ...
        if pending:
            with ThreadPoolExecutor(max_workers=min(MAX_UPSERT_CONCURRENCY, len(pending))) as pool:
                futures = [pool.submit(_write_one, item) for item in pending.items()]
