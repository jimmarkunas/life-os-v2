...
Prefer the smallest proof that can disprove the package. Do not create an acceptance service, QA datastore, second canonical ledger, test orchestrator, evidence datastore, scheduler, or new recovery architecture to enforce this standard. An explicitly Jim-approved lightweight TestKit is permitted under `tests/` when it consolidates duplicated test setup or external-boundary fakes used by at least two existing test consumers. It must remain test-only, contain no product policy, and must not introduce a custom runner, plugin system, datastore, scheduler, registry/configuration language, inheritance framework, or production import dependency.
...
8. **TESTS ARE A LIABILITY BUDGET.** Repository target is at most 100 collected tests. While above 100, no package—including a test-only package—may increase the collected-test count. Dedicated consolidation packages must reduce it materially. Product fixes may add or replace regression cases inside an existing contract, but may not grow the collected-test count while the repository is above 100. Once at/below 100, CI hard-fails above 100 unless Jim explicitly approves an exception.
...
11. **TEST CREATION GATE.** At/below the 100-test ceiling, each new collected test requires a commit trailer `Production-Critical-Test: <specific failure prevented>`. When a regression belongs to an already-covered contract boundary, add it as a named case inside that contract instead of creating another collected test or incident-shaped test file. Test count is never a quality metric.
...
14. **TWO-CONSUMER ABSTRACTION RULE.** Do not create a shared production abstraction, framework, dispatcher, base class, or generic helper layer until two active production consumers require it. One production consumer means keep the logic local. Test-only TestKit reuse follows the explicit rule in §6.1.
...
18. **PRODUCTION-FROZEN PACKAGE CONTRACT.** Test architecture, cleanup, documentation, or other non-production packages may declare `Production-Frozen: true` in the commit history. When declared, any production code/config/workflow change is a CI failure. Use this by default when simplifying development infrastructure around an accepted production path. A production-frozen package that discovers a needed production change must stop and rescope it as a separate product package; it may not silently remove the freeze.
19. **STOP CODES.** Hard-stop labels are `BASE_MOVED_STOP`, `LINE_BUDGET_STOP`, `TOKEN_BUDGET_STOP`, `TIMEBOX_STOP`, `TEST_BUDGET_STOP`, and `SCOPE_EXPANSION_STOP`. A stop is not a failure to perform; it is the required behavior when a budget is exhausted.
